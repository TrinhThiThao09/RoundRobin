...
public class MyFrame extends JFrame implements ActionListener {
	...
	private String authorName = "Trịnh Thị Thảo - K62CD - UET";
	private String[] columnInput = { "Process name", "Start time", "Run time" };
	private String[] strInput = { "Round Robin", "Process name", "Start time",
			"Run time" };
	...
	private JPanel createInputRoundRobin() {
		...
		panel.add(new JLabel("Round Robin"), BorderLayout.WEST);
		...
		if (checkInput()) {
			...
			if (roundRobinTime != 0
					&& roundRobinTime != Integer.parseInt(tfRoundRobin
							.getText().trim())) {
				int select = JOptionPane.showOptionDialog(null,
						"Are you want change round robin time?",
						"Change round robin time", JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE, null, null, null);
				if (select == 0) {
